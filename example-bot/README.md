...
| **Меню в одном сообщении: callback-кнопки + текст как единый диалог** | `dialog/FeedbackMenuDialog` (`/feedback`) |
...
### Callback-кнопки внутри диалога

Диалог принимает не только текст, но и нажатия inline-кнопок:

- `@DialogStep("имя")` — обрабатывает **текстовый** ввод на текущем шаге;
- `@DialogCallback("действие")` — обрабатывает нажатие кнопки `Callback.to("действие")`,
  пока диалог активен (матчится по имени действия, а не по шагу — у клавиатуры
  много кнопок сразу);
- `ctx.edit(text, keyboard)` — редактирует «якорное» сообщение (его id библиотека
  запоминает автоматически), так что и кнопки, и текстовые шаги меняют **одно**
  сообщение — это и есть «меню в одном сообщении».

`/feedback` показывает смешанный сценарий: меню → кнопка выбирает поле → диалог ждёт
текст → значение сохраняется → меню перерисовывается → «Закончить» завершает и
показывает сводку. Простые же self-contained кнопки (состояние влезает в
`callback_data`) по-прежнему делаются стейтлес `@CommandMapping(type=CALLBACK)` —
см. `controller/MenuController`.
